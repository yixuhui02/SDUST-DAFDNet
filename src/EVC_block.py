...
class Encoding(nn.Module):
    def __init__(self, in_channels, num_codes):  # in_channels、num_codes分别表示输入数据的通道数和编码向量的数量。
        super(Encoding, self).__init__()
        # init codewords and smoothing factor
        self.in_channels, self.num_codes = in_channels, num_codes
        num_codes = 64
        std = 1. / ((num_codes * in_channels)**0.5)
        # codewords 是一个形状为 [num_codes, in_channels]的张量，用于存储编码字典的参数，并且这些参数会随着训练过程中的反向传播进行调整。
        self.codewords = nn.Parameter(
            torch.empty(num_codes, in_channels, dtype=torch.float).uniform_(-std, std), requires_grad=True)
        # self.scale 是一个形状为 [num_codes] 的张量，用于调整编码特征的权重，它的值会随着训练过程中的反向传播进行更新。
